...
namespace UnityDonationAlerts
{
    ...
    public class LoopbackAccessTokenProvider : IAccessTokenProvider
    {
        ...
        private DonationAlertsSettings settings;
        private AuthCodeExchanger authCodeExchanger;
        private string codeVerifier;
        private string redirectUri;
        private string authorizationCode;

        public LoopbackAccessTokenProvider (DonationAlertsSettings donationAlertsSettings)
        {
            settings = donationAlertsSettings;
            unitySyncContext = SynchronizationContext.Current;

            var accessTokenRefresher = new AccessTokenRefresher(settings.GenericClientCredentials);
        ...
        public void ProvideAccessToken ()
        {
            ...
            // Access token will never expire: https://dev.DonationAlerts.com/docs/oauth-2
        ...
        private void HandleAccessTokenRefreshed (AccessTokenRefresher refresher)
        {
            if (refresher.IsError)
            {
                ...
                {
                    var message = "UnityDonationAlerts: Failed to refresh access token; executing full auth procedure.";
        ...
        private void HandleAuthCodeExchanged (AuthCodeExchanger exchanger)
        {
            ...
            {
                Debug.LogError("UnityDonationAlerts: Failed to exchange authorization code.");
                HandleProvideAccessTokenComplete(true);
            }
            else
            {
        ...
        private void HandleHttpListenerCallbackOnUnityThread (object state)
        {
            ...
            {
                Debug.LogError($"UnityDonationAlerts: OAuth authorization error: {context.Request.QueryString.Get("error")}.");
                HandleProvideAccessTokenComplete(true);
                return;
            }
            if (context.Request.QueryString.Get("code") == null)
            {
                Debug.LogError($"UnityDonationAlerts: Malformed authorization response. {context.Request.QueryString}");
                HandleProvideAccessTokenComplete(true);
                return;
            }
